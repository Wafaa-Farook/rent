...
const toggleSection = (sectionName) => {
  ...
  const fetchRentalHistory = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/renter/history/${renterId}`, {
  ...
  const fetchAvailableItems = async () => {
    try {
      const res = await fetch("http://localhost:5000/api/renter/available");
      const data = await res.json();
  ...
  const fetchRequests = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/renter/requests/${renterId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
...
const handleRequest = async (itemId) => {
  ...
  try {
    const res = await fetch("http://localhost:5000/api/renter", {
      method: "POST",
      headers: {
